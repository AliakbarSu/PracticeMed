<template>
  <div class="space-y-10 divide-y divide-gray-900/10">
    <div class="flex flex-wrap gap-x-8 gap-y-8 pt-10">
      <div
        :class="{
          'shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl': !partnerAdded,
        }"
        class="w-full p-5"
      >
        <Vueform
          ...
          :disabled="loading"
          :endpoint="false"
          class="vf-create-account"
          v-bind="vueform"
          w-full
          @submit="submit"
        />
        <div v-else class="m-auto text-center w-2/3">
          <h1
            class="mt-2 text-1xl font-bold tracking-tight text-gray-900 sm:text-3xl"
          >
            Thank you for submitting your details
          </h1>
          <div class="max-w-xl">
            <p class="mt-6">
              We will be in touch with you if someone is available.
            </p>
          </div>
        </div>
      </div>
...
import { addStudyPartner } from "../../../src/api/studyPartnersApi";
import { countries, timeZones, topics } from "./data";
...
const partnerAdded = ref(false);

const submit = async (form$: any) => {
  loading.value = true;
  const formData = form$.data;
  try {
    await addStudyPartner(formData);
    partnerAdded.value = true;
  } catch (e) {
    console.error(e);
  } finally {
    loading.value = false;
...
.vf-create-account:root {
  --vf-primary: #4f46e5;
  --vf-primary-darker: #443cde;
  --vf-color-on-primary: #ffffff;
  --vf-danger: #ef4444;
  --vf-danger-lighter: #fee2e2;
  --vf-success: #4f46e5;
  --vf-success-lighter: #635bec;
  --vf-gray-50: #f9fafb;
  --vf-gray-100: #f3f4f6;
  --vf-gray-200: #e5e7eb;
  --vf-gray-300: #d1d5db;
  ...
  --vf-color-success: var(--vf-primary);
