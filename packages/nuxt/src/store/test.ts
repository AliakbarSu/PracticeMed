...
export const useTestStore = defineStore("test", () => {
  ...
  const timeElapsed = computed(() => {
    const timeElapsed =
      100 -
      ((testDuration.value - timeRemained.value.mil) / testDuration.value) *
        100;
  ...
  const question = computed(() => {
    if (test.value) {
      const isLast =
        isLastQuestion.value && skippedQuestions.value.length === 0;
      return {
  ...
  const setDemoQuestions = () => {
    if (!test.value) return;
    test.value.questions = questions.value.slice(0, 11) as QuestionInProgress[];
  };
